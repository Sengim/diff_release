...
class ReadASCADv2:
    ...
    def load_dataset(self):
        ...
        metadata = in_file["Profiling_traces/metadata"]
        ind = permIndices(self.target_byte, metadata["masks"][:, 0], metadata["masks"][:, 1], metadata["masks"][:, 2], metadata["masks"][:, 3])
        plaintext = metadata["plaintext"][np.arange(len(metadata["plaintext"])), ind]
        key = metadata["key"][np.arange(len(metadata["key"])), ind]
        ...
        self.labels_key_hypothesis_validation = self.create_labels_key_guess(validation_plaintexts, validation_keys, validation_masks,
                                                                             leakage_order=order)
        self.labels_key_hypothesis_attack = self.create_labels_key_guess(attack_plaintext, attack_keys, attack_masks, leakage_order=order)
        if self.ns == 2000:
